...
void start_the_game(void)
{
	...
	while (level <= NUM_LEVELS) {
		clear(buf);
		textprintf_centre(buf, font, 160, 100, 15, "Level %d", level);
		textprintf_centre(buf, font, 160, 110, 15, "%d %s left", lives, (lives > 1) ? "lives" : "life");
		stretch_blit(
			buf,
			screen,
			0,
